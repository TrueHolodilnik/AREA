...
BOOL 		ps_mask_control				= 0; 
BOOL 		ps_drops_control			= 0;
BOOL 		ps_sslr_control				= 0;
BOOL 		ps_postprocess_control		= 0;
BOOL 		ps_chrom_abberation_control	= 0;
BOOL 		ps_bloom_control			= 0;
BOOL 		ps_dof_chrabb				= 0;
BOOL 		ps_sky_rainbow				= 0;
BOOL 		ps_water_ripples			= 0;
BOOL 		ps_water_mudfoam			= 0;
BOOL 		ps_metall_refls				= 0;
BOOL 		ps_vignette					= 0;
BOOL 		ps_fake_tonemap				= 0;

float		ps_techni_amount			= 0.0f;
float		ps_techni_power				= 0.0f;
float		ps_techni_red				= 0.0f;
float		ps_techni_green				= 0.0f;
float		ps_techni_blue				= 0.0f;
float		ps_chrom_abb_factor			= 0.0f;
float		ps_chrom_abb_power			= 0.0f;
float		ps_hdr_factor				= 0.0f;
float		ps_luma_factor				= 0.0f;
float		ps_luma_power				= 0.0f;
int			ps_luma_pattern				= 0.0f;
float		ps_luma_offset				= 0.0f;
float		ps_dpx_red					= 0.0f;
float		ps_dpx_green				= 0.0f;
float		ps_dpx_blue					= 0.0f;
float		ps_dpx_color_gamma			= 0.0f;
float		ps_dpx_saturation			= 0.0f;
float		ps_dpx_power				= 0.0f;
int			ps_curves_formula			= 0.0f;
float		ps_curves_contrast			= 0.0f;
int			ps_sslr_samples				= 0;
float		ps_vignette_power			= 0.0f;

float		ps_script_wetness			= 0.0f;
float		ps_r2_df_parallax_h			= 0.02f;
float		ps_r2_df_parallax_range		= 75.f;
float		ps_r2_tonemap_middlegray	= 1.3f;				// r2-only
float		ps_r2_tonemap_adaptation	= 10.f;				// r2-only
float		ps_r2_tonemap_low_lum		= 0.5f;				// r2-only
float		ps_r2_tonemap_amount		= 1.0f;				// r2-only
float		ps_r2_ls_bloom_kernel_g		= 3.f;				// r2-only
float		ps_r2_ls_bloom_kernel_b		= 1.0f;				// r2-only
float		ps_r2_ls_bloom_speed		= 50.f;				// r2-only
float		ps_r2_ls_bloom_kernel_scale	= .9f;				// r2-only	// gauss
float		ps_r2_ls_dsm_kernel			= .7f;				// r2-only
float		ps_r2_ls_psm_kernel			= .7f;				// r2-only
float		ps_r2_ls_ssm_kernel			= .7f;				// r2-only
float		ps_r2_ls_bloom_threshold	= .03f;				// r2-only
...
//	CMD3(CCC_Mask,		"r2_sun_shafts",				&ps_r2_ls_flags,			R2FLAG_SUN_SHAFTS);
	...
	CMD3(CCC_Token,		"r2_pom_quality",				&ps_pom_quality,			pom_token);
	...
	CMD4(CCC_Integer, "r__mask_control", &ps_mask_control, 0, 1); 
	CMD4(CCC_Integer, "r__drops_control", &ps_drops_control, 0, 1);
	CMD4(CCC_Float, "r__script_wetness", &ps_script_wetness, 0.f, 1.f);
	CMD4(CCC_Integer, "r__sslr_control", &ps_sslr_control, 0, 1);
	CMD4(CCC_Integer, "r__metall_refls", &ps_metall_refls, 0, 1);
	CMD4(CCC_Integer, "r__postprocess_control", &ps_postprocess_control, 0, 1);
	CMD4(CCC_Integer, "r__chrom_abb_control", &ps_chrom_abberation_control, 0, 1);
	CMD4(CCC_Integer, "r__bloom_control", &ps_bloom_control, 0, 1);
	CMD4(CCC_Integer, "r__dof_chrabb_control", &ps_dof_chrabb, 0, 1);
	CMD4(CCC_Integer, "r__sky_rainbow_control", &ps_sky_rainbow, 0, 1);
	CMD4(CCC_Integer, "r__water_ripp_control", &ps_water_ripples, 0, 1);
	CMD4(CCC_Integer, "r__mudfoam_control", &ps_water_mudfoam, 0, 1);
	CMD4(CCC_Integer, "r__vignette_control", &ps_vignette, 0, 1);
	CMD4(CCC_Integer, "r__fake_tonemap", &ps_fake_tonemap, 0, 1);

	CMD4(CCC_Float, "r__techni_amount", &ps_techni_amount, 0.01f, 1.f);
	CMD4(CCC_Float, "r__techni_power", &ps_techni_power, 0.2f, 8.f);
	CMD4(CCC_Float, "r__techni_red", &ps_techni_red, 0.f, 1.f);
	CMD4(CCC_Float, "r__techni_green", &ps_techni_green, 0.f, 1.f);
	CMD4(CCC_Float, "r__techni_blue", &ps_techni_blue, 0.f, 1.f);
	CMD4(CCC_Float, "r__chrom_abb", &ps_chrom_abb_factor, 0.f, 0.01f);
	CMD4(CCC_Float, "r__chrom_power", &ps_chrom_abb_power, 0.f, 3.f);
	CMD4(CCC_Float, "r__hdr_factor", &ps_hdr_factor, 0.f, 8.f);
	CMD4(CCC_Float, "r__luma_factor", &ps_luma_factor, 0.f, 1.f);
	CMD4(CCC_Float, "r__luma_power", &ps_luma_power, 0.f, 3.f);
	CMD4(CCC_Integer, "r__luma_pattern", &ps_luma_pattern, 1, 6);
	CMD4(CCC_Float, "r__luma_offset", &ps_luma_offset, 0.0f, 6.0f);
	CMD4(CCC_Float, "r__dpx_red", &ps_dpx_red, 0.2f, 0.6f);
	CMD4(CCC_Float, "r__dpx_green", &ps_dpx_green, 0.2f, 0.6f);
	CMD4(CCC_Float, "r__dpx_blue", &ps_dpx_blue, 0.2f, 0.6f);
	CMD4(CCC_Float, "r__dpx_color_gamma", &ps_dpx_color_gamma, 0.1f, 2.5f);
	CMD4(CCC_Float, "r__dpx_saturation", &ps_dpx_saturation, 0.f, 8.0f);
	CMD4(CCC_Float, "r__dpx_power", &ps_dpx_power, 0.f, 1.0f);
	CMD4(CCC_Integer, "r__curves_formula", &ps_curves_formula, 1, 12);
	CMD4(CCC_Float, "r__curves_contrast", &ps_curves_contrast, -1.f, 1.0f);
	CMD4(CCC_Integer, "r__sslr_samples", &ps_sslr_samples, 1, 15);
	CMD4(CCC_Float, "r__vignette_power", &ps_vignette_power, 1.0f, 4.0f);
