...
void	CRenderTarget::phase_accumulator()
{
	...
	if (dwAccumulatorClearMark==Device.dwFrame)	{
      ...
      {
		  if (RImplementation.o.fp16_blend)	u_setrt(rt_Accumulator, NULL, NULL, NULL, HW.pBaseZB);
		  else								u_setrt(rt_Accumulator_temp, NULL, NULL, NULL, HW.pBaseZB);
      }
      else
      {
		  if (RImplementation.o.fp16_blend)	u_setrt(rt_Accumulator, NULL, NULL, NULL, rt_MSAADepth->pZRT);
		  else								u_setrt(rt_Accumulator_temp, NULL, NULL, NULL, rt_MSAADepth->pZRT);
      }
	} else {
		// initial setup
...
void	CRenderTarget::phase_vol_accumulator()
{
	if (!m_bHasActiveVolumetric)
	{
		...
		if( !RImplementation.o.dx10_msaa )
			u_setrt(rt_Generic_2, NULL, NULL, NULL, HW.pBaseZB);
		else
			u_setrt(rt_Generic_2, NULL, NULL, NULL, RImplementation.Target->rt_MSAADepth->pZRT);
