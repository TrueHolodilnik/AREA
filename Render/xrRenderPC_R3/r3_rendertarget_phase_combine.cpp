...
//.		Fvector4	envclr			= { envdesc.sky_color.x*2+EPS,	envdesc.sky_color.y*2+EPS,	envdesc.sky_color.z*2+EPS,	envdesc.weight					};
   ...
   //phase_combine_refl();
	...
	if (ps_r2_ls_flags.test(R3FLAG_DYN_WET_SURF)) {
		...
		if (!RImplementation.o.dx10_msaa){
			HW.pDevice->CopyResource(rt_Generic_0->pTexture->surface_get(), rt_SSR->pTexture->surface_get());
			HW.pDevice->CopyResource(rt_LF->pTexture->surface_get(), rt_Generic_0->pTexture->surface_get());
		}
		else {
			HW.pDevice->CopyResource(rt_Generic_0_r->pTexture->surface_get(), rt_SSR->pTexture->surface_get());
			HW.pDevice->CopyResource(rt_LF->pTexture->surface_get(), rt_Generic_0_r->pTexture->surface_get());
		}
