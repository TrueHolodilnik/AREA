...
static class cl_m_v2w_r : public R_constant_setup
{
	virtual void setup(R_constant* C)
	{
		Fmatrix		m_v2w;
...
class cl_water_params : public R_constant_setup {
	virtual void setup(R_constant* C) {
		Fvector4	result;
		result.set(ps_sslr_control, ps_water_ripples, ps_water_mudfoam, ps_sslr_samples);
		RCache.set_c(C, result);
	}
};
static cl_water_params binder_water_params;
...
void					CRender::create					()
{
	...
	*/
	...
	RM->RegisterConstantSetup("m_v2w", &binder_v2w_r);
	...
	RM->RegisterConstantSetup("water_forward_params", &binder_water_params);
...
CRender::~CRender()
{
	for (FSlideWindowItem it : SWIs) // Memory leak fix
	{
		xr_free(it.sw);
		it.sw = nullptr;
		it.count = 0;
	...
	void*&							result)
{
	...
	if (ps_pom_quality)
	{
		xr_sprintf(c_pom, "%d", ps_pom_quality);
		defines[def_it].Name = "POM_QUALITY";
		defines[def_it].Definition = c_pom;
		def_it++;
		sh_name[len] = '0' + char(ps_pom_quality); ++len;
	}
	else
	{
		sh_name[len] = '0'; ++len;
