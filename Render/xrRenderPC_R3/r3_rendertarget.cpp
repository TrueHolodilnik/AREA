...
void	CRenderTarget::u_setrt(const ref_rt& _1, const ref_rt& _2, const ref_rt& _3, const ref_rt& _4 , ID3DDepthStencilView* zb)
{
	...
	if (_4) RCache.set_RT(_4->pRT, 3); else RCache.set_RT(NULL, 4);
	RCache.set_ZB							(zb);
...
void	CRenderTarget::u_setrt(const ref_rt& _1, ID3DDepthStencilView* zb)
{
	VERIFY(_1 || zb);
	if (_1)
	{
   ...
   if( RImplementation.o.dx10_gbuffer_opt )
	...
	//	NORMAL
	{
      ...
      if( !RImplementation.o.dx10_gbuffer_opt )
		...
		if (RImplementation.o.dx10_msaa) rt_PPTemp.create("$user$genericpp", w, h, D3DFMT_A8R8G8B8, SampleCount);
		else rt_PPTemp.create("$user$genericpp", w, h, D3DFMT_A8R8G8B8, 1);

		if (RImplementation.o.dx10_msaa) rt_LF.create(r2_RT_LF, w, h, D3DFMT_A8R8G8B8, SampleCount);
		else rt_LF.create(r2_RT_LF, w, h, D3DFMT_A8R8G8B8, 1);

		if (RImplementation.o.dx10_msaa) rt_SSR.create("$user$ssrrefls", w, h, D3DFMT_A8R8G8B8, SampleCount);
		else rt_SSR.create("$user$ssrrefls", w, h, D3DFMT_A8R8G8B8, 1);

		rt_secondVP.create      (r2_RT_secondVP, w, h, D3DFMT_A8R8G8B8, 1);		// +SecondVP+
		...
		if( RImplementation.o.dx10_msaa )
		{
			...
			//t_Generic.create	 (r2_RT_generic,w,h,   D3DFMT_A8R8G8B8, 1		);
		}
	...
	s_SSLR.create(b_SSLR, "r3\\sslr");

	// RAIN DROPS
	//s_rain_drops.create				(b_rain_drops,	"r3\\rain_drops");
