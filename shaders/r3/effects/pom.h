...
#define POM_H
	...
	//#define	POM_MIN_SAMPLES int(20) Nie tanet dazhe s 2070, pizdec
...
void update_texcoords(inout p_bumped I)
{
	if (I.position.z < fParallaxStopFade)
	{
		...
		[unroll(POM_MAX_SAMPLES)] for( ;fCurrHeight < fCurrentBound; fCurrentBound -= fStepSize )
		{
			vTexCurrentOffset += vTexOffsetPerStep;		
			fCurrHeight = s_bumpX.Sample( smp_base, float4(vTexCurrentOffset.xy,0,0)).a;
		}
