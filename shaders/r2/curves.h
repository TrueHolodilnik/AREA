...
half4 CurvesPass(half4 colorInput )
{
  float3 lumCoeff = float3(0.2126, 0.7152, 0.0722);  //Values to calculate luma with
  float Curves_contrast_blend = curves_sslr_params.y;
  ...
  if (curves_sslr_params.x == 5) {
    ...
    Curves_contrast_blend = curves_sslr_params.y * 2.0; //I multiply by two to give it a strength closer to the other curves.
  }
  ...
  */
  ...
  // -- Curve 11 --
  if (curves_sslr_params.x == 11) { //Cubic Bezier spline
  ...
  // -- Curve 12 --
  if (curves_sslr_params.x == 12) { //Cubic Bezier spline - alternative implementation.
