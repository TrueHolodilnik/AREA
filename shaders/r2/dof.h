...
half3	dof(float2 center)
{
	...
	float2 chromatic_abberation;
	if (pp_control_params.w) chromatic_abberation = center*chrom_hdr_luma_params.x;
	// poisson
	half2 	o  [12];
   	...
   	for (int i=0; i<12; i++)
	{
		...
		if (pp_control_params.w) {
			tap_color.r	= tex2D	(s_image,tap + chromatic_abberation).r;
			tap_color.b	= tex2D	(s_image,tap).g;
			tap_color.g	= tex2D	(s_image,tap - chromatic_abberation).b;
		}
		else {
			tap_color	= tex2D	(s_image,tap);
		}
